...
if(isadminloggedin()) {
  $options=array_merge($defaults,array('type' => 'object', 'subtype' => 'IzapSocialAuction', 'container_guid' => NULL));
  if($count=elgg_get_entities(array_merge($options,array('count'=>true))) ) {
  ...
  $options=array_merge(
    ...
    array('type' => 'object', 'subtype' => 'IzapSocialAuction', 'container_guid' => NULL, 'metadata_name_value_pairs'=>
      array(
        array('name'=>'expire_date','value'=>time(),'operand'=>'>=','case_sensitive'=>true),
