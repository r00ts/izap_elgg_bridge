...
return sEventName;},
addHandler:function(sElementId,sEvent,sFunctionName){if(window.addEventListener){sEvent=this.stripOnPrefix(sEvent);eval("this.proto('"+sElementId+"').addEventListener('"+sEvent+"',"+sFunctionName+",false);");}
else{sAltEvent=this.addOnPrefix(sEvent);eval("this.proto('"+sElementId+"').attachEvent('"+sAltEvent+"',"+sFunctionName+",false);");}
},
removeHandler:function(sElementId,sEvent,sFunctionName){if(window.addEventListener){sEvent=this.stripOnPrefix(sEvent);eval("this.proto('"+sElementId+"').removeEventListener('"+sEvent+"',"+sFunctionName+",false);");}
else{sAltEvent=this.addOnPrefix(sEvent);eval("this.proto('"+sElementId+"').detachEvent('"+sAltEvent+"',"+sFunctionName+",false);");}
},
create:function(sParentId,sTag,sId){var objParent=this.proto(sParentId);objElement=document.createElement(sTag);objElement.setAttribute('id',sId);if(objParent)
objParent.appendChild(objElement);},
insert:function(sBeforeId,sTag,sId){var objSibling=this.proto(sBeforeId);objElement=document.createElement(sTag);objElement.setAttribute('id',sId);objSibling.parentNode.insertBefore(objElement,objSibling);},
insertAfter:function(sAfterId,sTag,sId){var objSibling=this.proto(sAfterId);objElement=document.createElement(sTag);objElement.setAttribute('id',sId);objSibling.parentNode.insertBefore(objElement,objSibling.nextSibling);},
...
return Obj;},
createInput:function(sParentId,sType,sName,sId){var objParent=this.proto(sParentId);var objElement=this.getInput(sType,sName,sId);if(objParent&&objElement)
objParent.appendChild(objElement);},
insertInput:function(sBeforeId,sType,sName,sId){var objSibling=this.proto(sBeforeId);var objElement=this.getInput(sType,sName,sId);if(objElement&&objSibling&&objSibling.parentNode)
objSibling.parentNode.insertBefore(objElement,objSibling);},
insertInputAfter:function(sAfterId,sType,sName,sId){var objSibling=this.proto(sAfterId);var objElement=this.getInput(sType,sName,sId);if(objElement&&objSibling&&objSibling.parentNode){objSibling.parentNode.insertBefore(objElement,objSibling.nextSibling);}
},
remove:function(sId){objElement=this.proto(sId);if(objElement&&objElement.parentNode&&objElement.parentNode.removeChild){objElement.parentNode.removeChild(objElement);}
},
replace:function(sId,sAttribute,sSearch,sReplace){var bFunction=false;if(sAttribute=="innerHTML")
sSearch=this.getBrowserHTML(sSearch);eval("var txt=this.proto('"+sId+"')."+sAttribute);if(typeof txt=="function"){txt=txt.toString();bFunction=true;}
if(txt.indexOf(sSearch)>-1){var newTxt='';while(txt.indexOf(sSearch)>-1){x=txt.indexOf(sSearch)+sSearch.length+1;newTxt+=txt.substr(0,x).replace(sSearch,sReplace);txt=txt.substr(x,txt.length-x);}
newTxt+=txt;if(bFunction){eval('this.proto("'+sId+'").'+sAttribute+'=newTxt;');}
else if(this.willChange(sId,sAttribute,newTxt)){eval('this.proto("'+sId+'").'+sAttribute+'=newTxt;');}
}
},
...
data="";if(cmd!="jc")objElement=this.proto(id);var cmdFullname;try{if(cmd=="cc"){cmdFullname="confirmCommands";var confirmResult=confirm(data);if(!confirmResult){skipCommands=id;}
}
...
else if(cmd=="ev"){cmdFullname="addEvent";property=this.addOnPrefix(property);eval("this.proto('"+id+"')."+property+"= function(){"+data+";}");}
...
elementObject=this.proto(element);if(elementObject){var oldData;eval("oldData=this.proto('"+element+"')."+attribute);if(newData!==oldData)
return true;}
return false;},
