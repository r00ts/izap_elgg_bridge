...
//    this.builder = new pfcGuiBuilder();
  ...
  setTabById: function(tabid)
  {
    ...
    for (var i=0; i<this.tabids.length; i++)
    {
      var tabtitle   = proto('pfc_channel_title'+this.tabids[i]);
      var tabcontent = proto('pfc_channel_content'+this.tabids[i]);
  ...
  getChatContentFromTabId: function(tabid)
  {
    ...
    // return the pfchat content if it exists
    var cc = this.chatcontent.get(tabid);
    if (cc) return cc;

    // if the pfchat content doesn't exists yet, just create a cached one
  ...
  {
    // remove the widgets
    var tabparent_t = proto('pfc_channels_list');
    var tabparent_c = proto('pfc_channels_content');
    var tab_t = proto('pfc_channel_title'+tabid);
    var tab_c = proto('pfc_channel_content'+tabid);
    tabparent_t.removeChild(tab_t);
    tabparent_c.removeChild(tab_c);

    // empty the pfchat div content
  ...
  */
  ...
  createTab: function(name, tabid, type)
  {
    ...
    proto('pfc_channels_list').appendChild(li_title);
    proto('pfc_channels_content').appendChild(div_content);

    // force the height of the pfchat/online zone in pixel in order fix blank screens on IE6
    div_chat.style.height   = (proto('pfc_channels_content').offsetHeight-1)+'px';
    div_online.style.height = (proto('pfc_channels_content').offsetHeight-1)+'px';
  ...
  notifyTab: function(tabid)
  {
    ...
    var img = proto('pfc_tabimg'+tabid);
    if (img)
    {
      if (tabtype == 'ch')
    ...
    var div = proto('pfc_tabdiv'+tabid);
    if (div)
    {
  ...
  {
    var container = proto('pfc_smileys');
